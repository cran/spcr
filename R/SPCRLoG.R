...
SPCRLoG <- function(x, y, k, xi, w, A, gamma0, gamma, Beta, lambda_gamma, lambda_beta){
	...
	while( max(abs(PARA_new-PARA_old)[PARA_new-PARA_old != 0]) > 1e-4 )
	{
		...
		ptilde <- 1 - 1 / (1 + exp( gamma0 + x%*%Beta%*%gamma ) )
		eta <- ptilde*( 1 - ptilde )
		z <- gamma0 + x%*%Beta%*%gamma + ( y - ptilde )/eta
	...
	return( list( gamma0=gamma0, gamma=gamma, Beta=Beta, A=A ) )
}

####### SPCR for Logistic regression (adaptive)
adaSPCRLoG <- function(x, y, k, q, xi, w, A, gamma0, gamma, Beta, lambda_gamma, lambda_beta, BetaWeight){
...
CV.SPCRLoG <- function(x, y, k, xi, w, nfolds=5, lambda.beta.candidate, lambda.gamma.candidate, center=TRUE, scale=FALSE, adaptive=FALSE, q=1){
...
#					spcr.object <- .Call( "spcr", x, y, A, Beta, gamma, gamma0, lambda.beta, lambda.gamma, xi, w )
					spcr.object <- SPCRLoG( x=x, y=y, A=A, k=k, gamma0=gamma0, gamma=gamma, Beta=Beta, lambda_beta=lambda.beta, lambda_gamma=lambda.gamma, xi=xi, w=w )
...
#					BetaWeight <- Beta
					...
					adaspcr.object <- adaSPCRLoG( x=x, y=y, A=A, k=k, q=q, gamma0=gamma0, gamma=gamma, Beta=Beta, lambda_beta=lambda.beta, lambda_gamma=lambda.gamma, xi=xi, w=w, BetaWeight=BetaWeight )
				...
				### CV-error
				s_cv <- ( gamma0*sum(y.test.cv) + t(y.test.cv)%*%x.test.cv%*%Beta%*%gamma - sum( log(1 + exp(gamma0 + x.test.cv%*%Beta%*%gamma)) ) )/length(y.test.cv)
	...
	return( list(CV.mat = CV.mat, lambda.beta.candidate = lambda.beta.candidate, lambda.gamma.candidate = lambda.gamma.candidate, lambda.gamma.cv = lambda.gamma.candidate[ whichimaxCandi.row ], lambda.beta.cv = lambda.beta.candidate[ whichimaxCandi.col[ which.max(maxCandi.col) ] ], cvm=max(maxCandi.row) ) )
}
